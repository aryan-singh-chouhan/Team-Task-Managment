...
const TaskBoard = React.memo(({ tasks, onStatusChange, onEdit, onDelete, currentUserId, isAdmin }) => {
  ...
  return (
    <div className="w-full">
          ...
          {tabs.map(tab => {
            ...
            return (
              ...
              >
                ...
                <span className={`inline-flex items-center justify-center min-w-[18px] h-5 px-1 rounded-full text-[10px] font-bold ${
                  isActive
      ...
      <div className="animate-fade-in">
        {renderTaskList(tabs.find(t => t.id === activeTab)?.tasks || [])}
      </div>
    </div>
  );
});
